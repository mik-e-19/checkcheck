...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/25 00:00:00 by kolim             #+#    #+#             */
...
{
	const char msg[] = "Input must be between 0 and 10 (exclusive)!\n";

	write(2, msg, sizeof(msg) - 1);
}

void	ft_prn_combn_re(int i_num_digit, int i_digit_pos, int i_ctr, int i_nb)
{
	int	nb;
	...
	while (i_ctr <= 9)
	{
		...
		if (i_digit_pos == 1)
		{
			...
			ft_print_number(nb, i_num_digit);
		}
		else
		{
			ft_prn_combn_re(i_num_digit, i_digit_pos - 1, i_ctr + 1, nb);
		}
		i_ctr++;
	}
}
